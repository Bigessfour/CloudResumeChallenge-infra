...
| **Lambda validation** | Fail-closed allowlist. Rejects empty/bot User-Agents (curl, scrapers, HeadlessChrome / DevTools agents), missing allowlisted `Origin`, and requests without browser `Sec-Fetch-Site` / `Sec-Fetch-Mode`. Referer alone is not enough. Rejected calls return **403** and **do not increment**. |
